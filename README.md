...
Role	    View Internal AI Notes	View Staff/Clinician      NotesEdit Own Notes	 View Patient-Facing Only
Patient	  ❌ Blocked at DB level	❌ Blocked at DB level	  ❌ (Read-only)	     ✅ Yes
Staff	    ❌ Blocked at DB level	✅ Yes (own clinic scope)	✅ Yes	             ❌ No
Clinician	✅ Yes	                ✅ Yes (full view)	      ✅ Yes	             ❌ No
Admin	    ✅ Yes (clinic-scoped)	✅ Yes (clinic-scoped)	  ✅ Yes              ❌ No

How It Works
Authentication: JWT token (or session) is decoded to extract user_id and role.

Authorization: Every API endpoint that returns timeline entries or notes passes the current user context to the database query, which adds a WHERE clause to filter out sensitive fields (is_sensitive = True for patients) or restricts access based on clinic_id and author_role rules.

Test Coverage: See tests/test_rbac_scope.py for automated assertions.
...
🎥 Demo Scenarios
Glance View & AI Scribe Integration: Open patient page → See Top Card in <200ms → Click highlight to jump to source timeline entry.

Collaborative Audit Trail: Staff adds note with @clinician tag → Clinician edits plan → Revision history shows diffs → Revert to previous version.
